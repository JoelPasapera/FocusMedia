...
_SECRETOS = (
    ...
    # La comilla de cierre de la clave va DENTRO del separador: en JSON
    # queda «"sessionid":», y exigiendo el separador pegado a la clave el
    # patrón no casaba. Los volcados de error guardan cuerpos JSON, así
    # que ahí se escapaba entero un sessionid reutilizable.
    re.compile(r"\b(sessionid|csrftoken|csrf_token|mid|ig_did|authorization"
               r"|set-cookie|x-csrftoken|access_token|fb_dtsg|lsd)"
               r"(['\"]?\s*(?:[=:]|%3D)\s*)(['\"]?)([^;,&\s'\"}]+)", re.I),
    # «Bearer xyz» lleva un espacio dentro del valor, así que cortar en el
    # primer blanco dejaba el token suelto detrás del <oculto>.
...
def limpiar_secretos(texto: str) -> str:
    ...
    # «authorization: Bearer xyz» lleva un espacio DENTRO del valor, así
    # que el patrón de arriba corta en el blanco y deja el token suelto
    # detrás. Va aparte, conservando la clave para que se vea qué se
    # ocultó.
    limpio = _AUTORIZACION.sub(r"\1\2\3<oculto>", limpio)
