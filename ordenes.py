...
def cmd_historial(args) -> None:
    if not motor.CARPETA.exists():
        sys.exit(f"No existe '{motor.CARPETA.name}'. Ejecuta 'bajar' primero.")
...
def cmd_contar(_) -> None:
    ...
    if p.get("publicaciones"):
        ...
        print(f"  {'Publicaciones':12} {p['publicaciones']}{cambio}")
    if p["privada"]:
...
def cmd_inspeccionar(args) -> None:
    ...
    motor.escribir_atomico(crudo, json.dumps(d, indent=2, ensure_ascii=False))
...
def cmd_importar(args) -> None:
    ...
    if r["con_sello"]:
        ...
        print("  Si ese rango cuadra con cuándo te siguió la gente, es la")
        print("  fecha del follow. Si son todas de hace un rato, es la fecha")
        print("  en que se generó el export y no sirve para nada.")
    else:
