...
def w1_arranque():
    ...
    check(len(v.botones_accion) == 18, f"18 acciones ({len(v.botones_accion)})")
    check(len(v.etiquetas_coste) == 18, "cada acción muestra su coste")
...
def g13_columnas_legibles():
    ...
    check(g._etiqueta_columna("le_sigue") == "le sigue",
          "y los guiones bajos se van aunque no estén en la lista")
...
def w25_avatar_de_la_cuenta():
    ...
    Image.new("RGB", (150, 150), (90, 120, 160)).save(
        m.ruta_foto("cuenta.ejemplo"), "JPEG")
    v._refrescar_disco()
    v.update()
...
def w31_gestor_de_sesiones():
    ...
    caja = [w for w in v._pegar_sesion.winfo_children()
    ...
    v.accion_pegar_sesion()
    v.update()
    caja2 = [w for w in v._pegar_sesion.winfo_children()
             if isinstance(w, ctk.CTkTextbox)][0]
...
def w33_foco_de_teclado():
    ...
    v.campo_objetivo.event_generate("<FocusIn>")
    v.update()
    check(str(v.campo_objetivo.cget("border_color")) == g.ACENTO,
          "también en lo que se escribe")
...
def w34_escape_cierra():
    ...
    for abrir, cual in ((v.accion_ajustes, "_panel_ajustes"),
        ...
        v.update()
        check(ventana.winfo_exists() == 0, "y Escape la cierra")
    v.destroy()
...
def w37_las_ventanas_caben():
    ...
    v.accion_pegar_sesion()
    v.update()
    hijos = v._pegar_sesion.winfo_children()
    desplazables = [h for h in hijos if isinstance(h, ctk.CTkScrollableFrame)]
