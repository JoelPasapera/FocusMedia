...
def _seccion_relaciones(filas: list, leyenda: dict) -> str:
    """
    Quién es mutuo y quién no, con lo que significa cada palabra.

    La leyenda llega desde el motor y no se escribe aquí: copiada en dos
    sitios se separaría en el primer matiz que se corrigiera, y entonces
    la tabla y el informe dirían cosas distintas de lo mismo.
    """
    if not filas:
        return ""
    cuenta: dict = {}
    for f in filas:
        cuenta[f["relacion"]] = cuenta.get(f["relacion"], 0) + 1

    items = "".join(
        f'<li><code>{html.escape(k)}</code> — {html.escape(v)}'
        f' <span class="tenue">({cuenta.get(k, 0)})</span></li>'
        for k, v in leyenda.items() if k in cuenta)

    cuerpo = "".join(
        f'<tr><td><code>{html.escape(str(f.get("relacion", "")))}</code></td>'
        f'<td>{html.escape(str(f.get("username", "")))}</td>'
        f'<td>{html.escape(str(f.get("nombre", "")))}</td></tr>'
        for f in filas)

    return f"""
<section>
  <h2>Relaciones</h2>
  <ul class="leyenda">{items}</ul>
  <div class="tabla-envoltorio">
    <table id="tabla-relaciones">
      <thead><tr><th>relación</th><th>usuario</th><th>nombre</th></tr></thead>
      <tbody>{cuerpo}</tbody>
    </table>
  </div>
</section>
...
.leyenda {{ list-style: none; padding: 0; margin: 0 0 16px;
  font-size: 13px; color: {dim}; line-height: 1.9; }}
.leyenda code {{ display: inline-block; min-width: 200px; }}
.leyenda code {{ color: {texto}; background: {raised}; padding: 1px 6px;
  border-radius: 4px; }}
.tenue {{ color: {dim}; opacity: .7; }}
...
function pintar() {
  ...
  // Solo los de la tabla de listas. Antes era todo el documento, y al
  // añadir la tabla de relaciones sus encabezados quedaban enganchados a
  // este ordenador: pulsar uno habría reordenado la OTRA tabla por una
  // columna que no existe en ella.
...
{_seccion_relaciones(datos.get("relaciones") or [],
                     datos.get("leyenda_relacion") or {})}
