...
def contrastar(enriquecido: dict, trayectorias: dict) -> dict:
    """
    ¿El `timestamp` del export es de verdad la fecha del follow?

    Nadie lo ha comprobado nunca, y el propio código guarda el campo como
    `te_sigue_desde`, que es afirmarlo. Esto lo contrasta con lo ÚNICO que
    puede desmentirlo: tu propio historial.

    Dos pruebas, y la primera es la que decide:

    1. **Imposibles.** Si el sello es la fecha del follow, no puede ser
       POSTERIOR a la primera captura en la que ya aparecía esa persona:
       estaba ahí antes de la fecha en la que supuestamente empezó a
       seguirte. Uno puede ser un desfase de husos horarios; muchos
       significan que el sello es otra cosa.

    2. **Dispersión.** Fechas de follow reales se reparten por meses o
       años. Si casi todas caen en el mismo día, el sello es la fecha en
       que se generó el export —o la de la migración de Meta, o cualquier
       otra— y no la de la relación.

    Función pura: recibe los dos diccionarios y no toca disco.
    """
    desde = enriquecido.get("desde") or {}
    personas = {p.get("username"): p
                for p in (trayectorias.get("personas") or {}).values()
                if p.get("username")}

    cotejables, imposibles, ejemplos = 0, 0, []
    for usuario, ficha in desde.items():
        sello = ficha.get("te_sigue_desde") or ficha.get("le_sigues_desde")
        conocida = personas.get(usuario)
        if not (sello and conocida and conocida.get("primera")):
            continue
        cotejables += 1
        if sello > conocida["primera"]:
            imposibles += 1
            if len(ejemplos) < 5:
                ejemplos.append(f"{usuario}: export dice {sello}, ya estaba "
                                f"el {conocida['primera']}")

    fechas = [f for ficha in desde.values()
              for f in (ficha.get("te_sigue_desde"),
                        ficha.get("le_sigues_desde")) if f]
    distintas = sorted(set(fechas))
    mas_repetida = max((fechas.count(f) for f in distintas), default=0)

    # El veredicto solo se da cuando hay con qué. Decir «parece correcto»
    # tras cotejar tres personas sería el mismo error que este proyecto
    # evita en «publicar y crecer».
    if cotejables < 10:
        veredicto = "sin_datos"
    elif imposibles > cotejables * 0.05:
        veredicto = "no_es_la_fecha_del_follow"
    elif len(distintas) < 3 or mas_repetida > len(fechas) * 0.5:
        veredicto = "sospechoso_todas_el_mismo_dia"
    else:
        veredicto = "compatible"

    return {"veredicto": veredicto, "cotejables": cotejables,
            "imposibles": imposibles, "ejemplos": ejemplos,
            "fechas_distintas": len(distintas),
            "mas_repetida": mas_repetida, "con_sello": len(fechas),
            "desde": distintas[0] if distintas else "",
            "hasta": distintas[-1] if distintas else ""}
