...
class Trabajador:
    ...
    def _envolver(self, nombre, funcion, args, kwargs) -> None:
        ...
        except m.ErrorPrevisto as e:
            # Estos no son fallos del programa: son cosas que pasan y que
            # ya llevan su explicación dentro. Enseñar una traza de Python
            # con rutas de archivos encima no añade nada y asusta.
            salida.flush()
            registro.anotar(f"{type(e).__name__}: {e}", nivel="warning")
            self.cola.put(("error", str(e)))
        except Exception as e:
            salida.flush()
            registro.excepcion(f"error no previsto en «{nombre}»")
...
class VentanaTabla(ctk.CTkToplevel):
    ...
    def _tabla(self) -> None:
        ...
        self.marco_filtros.grid(row=2, column=0, sticky="ew", padx=12,
                                pady=(6, 0))

        contenedor = ctk.CTkFrame(self, corner_radius=8, fg_color=PANEL,
                                  border_width=1, border_color=LINE)
        contenedor.grid(row=3, column=0, sticky="nsew", padx=12, pady=10)
    ...
    def _leyenda(self) -> None:
        """
        Explica los valores de la columna 'relacion', si la hay.

        «mutuo», «solo_sigue_a_la_cuenta» y «solo_la_cuenta_le_sigue» no se
        explican solos, y sin la explicación la tabla enseña tres palabras
        y ninguna respuesta. Solo aparece en el archivo que la necesita.
        """
        if getattr(self, "banda_leyenda", None) is not None:
            self.banda_leyenda.destroy()
            self.banda_leyenda = None
        if "relacion" not in (self.columnas or []):
            return

        vistos = {f.get("relacion") for f in self.filas}
        # Con el nombre real de la cuenta dentro: «la cuenta» obligaba a
        # traducir mentalmente en cada línea.
        cuenta = ""
        if self._archivos:
            nombre = self._archivos[self.selector.get() and 0].stem
            cuenta = nombre.replace("relaciones_", "")
        leyenda = m.leyenda_relacion(cuenta)
        partes = [f"{k:<24}{v}" for k, v in leyenda.items() if k in vistos]
        if not partes:
            return
        # Una por línea y alineadas: en una sola línea con separadores las
        # tres frases se mezclaban y había que leerlas enteras para
        # separarlas.
        self.banda_leyenda = ctk.CTkLabel(
            self, text="\n".join(partes), anchor="w", justify="left",
            text_color=DIM, font=ctk.CTkFont(self.mono, TAM_MICRO))
        self.banda_leyenda.grid(row=1, column=0, sticky="ew",
                                padx=14, pady=(8, 0))
    ...
    def _filtrar_pronto(self) -> None:
        ...
        if self._pendiente:
            try:
                ...
                pass          # el temporizador ya había saltado: es normal
...
class Ventana(ctk.CTk):
    ...
    def __init__(self):
        ...
        self._log("  2. Pulsa «Importar sesión». No pide contraseña: "
    ...
    def _atar_foco(self, w) -> bool:
        ...
        def enganchar(destino) -> bool:
            try:
                destino.bind("<FocusIn>", entrar, add="+")
                destino.bind("<FocusOut>", salir, add="+")
                return True
            except Exception:
                return False

        if not enganchar(w):
            # Algunas versiones de customtkinter rechazan `bind` en los
            # widgets compuestos, y quien recibe el foco de verdad es el
            # campo de dentro. Antes esto devolvía False y el anillo no se
            # ponía: silencioso, y solo se vio ejecutando en Windows.
            interno = getattr(w, "_entry", None) or getattr(w, "_canvas",
                                                            None)
            if interno is None or not enganchar(interno):
                return False
    ...
    def _cabe_en_pantalla(self, ventana, ancho: int, alto: int) -> None:
        ...
        try:
            ...
            pass          # sin pantalla todavía: se queda el tamaño pedido
    ...
    def accion_estudio(self) -> None:
        """Cohortes, permanencia y composición. 0 peticiones."""
        if not self._preparar():
            return
        self._lanzar("Estudiando lo descargado", m.cmd_estudio,
                     _Args(lista=self.campo_lista.get()
                           if self.campo_lista.get() in ("seguidores",
                                                         "seguidos")
                           else "seguidores",
                           tope_hueco=7, incluir_sospechosas=bool(
    ...
    def _aviso_de_fallo(self, resumen: str) -> None:
        ...
        try:
            ...
            pass          # último recurso: si avisar falla, no se insiste
    ...
    def _mostrar_novedades(self) -> None:
        ...
        try:
            ...
            # Si esto falla, las mismas novedades vuelven a salir mañana y
            # pasado, y nadie sabría por qué. Callarlo era el único de
            # estos «except» con una consecuencia que se ve.
            registro.excepcion("no se pudieron marcar las novedades vistas")
            self._log("  (no se han podido marcar como vistas; volverán a "
                      "salir)", DIM)
    ...
    def _poner_icono(self) -> None:
        ...
        try:
            ...
            pass          # sin icono se vive; es lo único que se pierde
    ...
    def _lateral(self) -> None:
        ...
        self._boton_lateral(lateral, "Medir página", self.accion_pagina,
                            "Cuánta gente sirve Instagram por petición. Si "
                            "sirve más de 50, bajar las listas costaría "
                            "menos.", "2")
        ...
        self._boton_lateral(lateral, "Historial", self.accion_historial,
                            ...
                            "0")
        self._boton_lateral(lateral, "Estudio", self.accion_estudio,
                            "De los que entraron cada mes, cuántos siguen; "
                            "cuánto dura la gente; cómo es tu audiencia. "
                            "Todo sobre lo ya descargado.", "0")
    ...
    def _guiar(self, accion: str) -> None:
        ...
        if accion == "sesion":
            ...
            if dias >= 2:
                lineas = [
                    ("Ya tienes descargas de dos días o más, así que:",
                     None),
            ...
            lineas = [
                ("Si sirve más de 50, se cambia en «Ajustes» -> por_pagina.",
                 None),
                ("  Ahorra peticiones a cambio de que cada una se parezca "
                 "menos a las", DIM),
                ("  de un navegador. Con el doble no suele compensar.", DIM)]

        elif accion == "estudio":
            lineas = [
                ("Las cohortes son lo que Instagram no te enseña: no "
                 "cuántos tienes, sino", None),
                ("  cuántos de los de cada mes siguen contigo.", DIM),
                ("  Si hay huecos grandes en el calendario, lo de arriba "
                 "se mide sobre ellos.", DIM)]
    ...
    def _refrescar_disco(self) -> None:
        ...
        # «dias» cuenta fechas distintas, no archivos: comparar necesita
        # dos DÍAS y una descarga deja dos capturas. Se añadió a
        # estado_de_cuenta() en la v9.2 y no se trajo aquí, así que la
        # guía leía 0 y no ofrecía nunca el análisis.
        self._resumen = {"capturas": 0, "dias": 0, "pendiente": False,
                         "incompletas": False}
    ...
    def _normalizar_cuenta(self) -> None:
        ...
        if problema:
            ...
            self._log("Ahora pulsa «Importar sesión» para tomar la "
    ...
    def accion_pagina(self) -> None:
        """Mide cuánto sirve Instagram por página. Una petición por tamaño."""
        if not self._preparar():
            return
        # Hasta 100 y no más: pedir tamaños raros es lo que no se parece a
        # un navegador, y eso no puede quedar a un clic de distancia. Quien
        # quiera probar 200 lo pide por la línea de órdenes, que es donde
        # el aviso se lee antes de gastar nada.
        self._lanzar("Midiendo cuánto sirve por página", m.cmd_pagina,
                     _Args(lista="seguidores", hasta=100), accion="pagina")
        ...
        for tarea in (getattr(self, "_tarea_cola", None),
            ...
            if tarea:
                try:
                    ...
                    pass      # cerrando: puede haber saltado ya
